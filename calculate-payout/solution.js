const calculatePayout = (heads, tails, winner) => {
  ...
  const prefix = `The outcome is ${winner}, In total I have to pay ${payout.toFixed(2)}$ to the winner`;
  ...
  return totalEarned > 0
    ...
    : `${prefix}, of which ${Math.abs(totalEarned).toFixed(2)}$ from my own pockets :(`;
};
